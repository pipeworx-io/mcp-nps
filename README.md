...
`tools/list` at `https://gateway.pipeworx.io/nps/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/list_parks \
  -H 'Content-Type: application/json' \
  -d '{"query":"Yellowstone"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/list_parks`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
